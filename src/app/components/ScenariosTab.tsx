        ...
        <div className="bg-[#0f1629] rounded-xl p-6 mb-4 border border-slate-800">
          ...
          <div className="max-w-xs">
            ...
            <input
              ...
              onChange={(e) => actions.updateProfile({ birthDate: e.target.value })}
              ...
              {scenarios.map(scenario => (
                <div key={scenario._id} className={`flex items-center justify-between p-4 rounded-lg border ${scenario.isSelected ? 'bg-slate-800/30 border-slate-700' : 'bg-slate-900/20 border-slate-700/50 opacity-60'}`}>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <button onClick={() => actions.toggleSelected(scenario._id)} className={`w-5 h-5 rounded border-2 flex items-center justify-center ${scenario.isSelected ? 'border-emerald-400 bg-emerald-400/20' : 'border-slate-500'}`}>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {scenarios.length > 1 && (
                      <>
                        <button onClick={() => actions.moveScenario(scenario._id, "up")} disabled={scenarios.indexOf(scenario) === 0} className="p-1.5 text-slate-400 hover:text-slate-200 rounded disabled:opacity-30 disabled:cursor-not-allowed" title="Move up">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
                        </button>
                        <button onClick={() => actions.moveScenario(scenario._id, "down")} disabled={scenarios.indexOf(scenario) === scenarios.length - 1} className="p-1.5 text-slate-400 hover:text-slate-200 rounded disabled:opacity-30 disabled:cursor-not-allowed" title="Move down">
                    ...
                    <button onClick={() => actions.duplicateScenario(scenario._id)} className="p-1.5 text-slate-400 hover:text-slate-200 rounded" title="Duplicate">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                    </button>
                    {scenarios.length > 1 && (
                      <button onClick={() => confirm(`Delete "${scenario.name}"?`) && actions.deleteScenario(scenario._id)} className="p-1.5 text-slate-400 hover:text-red-400 rounded" title="Delete">
